...
app.post(
  ...
  body("phone_num")
    ...
    .withMessage("Phonenumber is required and has to a number"),
  body("password")
    .isLength({ min: 7 })
    .withMessage("password is required and must be at least 7 characters"),
  async (req, res) => {
    try {
      ...
      let user = await User.findOne({ phone_num: req.body.phone_num })
        .lean()
        .exec();
